...
// @notice       使用切换原文功能可能造成意想不到的结果，特别是当你同时使用其它脚本之后，切换回原文会造成其它脚本的大部分功能失效
...
var list = [], translate = true, change, added;
load();//加载翻译字典
build();//转换字典
initRestore();//原文切换按钮
main();//执行汉化
...
function build() {
    ...
    regEquips = {};
    for(var j in equips) {
        var vj = j.replace(/^of the /,'of\\s*(<[^<]+>\\s*)*the\\s*(<[^<]+>\\s*)*\\s*').replace(/^of ([^<\\])/,'of\\s*(<[^<]+>\\s*)*$1');
        regEquips[j] = new RegExp(vj,'gi');
    }
}
